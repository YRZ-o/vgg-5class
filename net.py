...
# pretrained 是一个布尔参数，用于指定是否加载预训练的模型权重。
# progress 是一个布尔参数，用于控制是否显示加载预训练模型时的进度条。
# num_classes 是一个整数参数，用于指定模型的输出类别数，默认值为 2。
def vgg16(pretrained=False, progress=True,num_classes=5):
    ...
    # 如果pretrained参数为True，这意味着用户想要加载预训练的模型权重。
    # load_state_dict_from_url函数用于从指定的URL加载预训练的模型权重，并返回一个状态字典。
    # model_dir是模型权重文件的存储路径。
    # model.load_state_dict(state_dict)将预训练的权重加载到模型中。
    if num_classes != 1000:
