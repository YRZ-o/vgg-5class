...
from data import *
'''数据集'''
annotation_path = '5catdogchikenhorsebutterfly.txt'  # 读取数据集生成的文件
with open(annotation_path, 'r') as f:
    lines = f.readlines()
np.random.seed(10101)# 函数用于生成指定随机数
np.random.shuffle(lines)# 数据打乱
np.random.seed(None)
num_val = int(len(lines)*0.2)# 十分之二的数据用来测试
num_train = len(lines)-num_val
# 输入图像大小
input_shape = [224,224]   # 导入图像大小
train_data = DataGenerator(lines[:num_train],input_shape,True)# true表示生成训练数据
val_data = DataGenerator(lines[num_train:],input_shape,False)# false表示生成测试数据
val_len = len(val_data)# 测试集长度
...
net=vgg16(True, progress=True,num_classes=5)# 定于分类的类别
net.to(device)
'''选择优化器和学习率的调整方法'''
lr=0.0001# 定义学习率
...
for epoch in range(epochs):
    ...
    for data in gen_train:
        ...
        optim.zero_grad()# 使用 zero_grad() 清除之前的梯度，以便进行新的前向传播。
        output = net(img)# 使用网络 net 对图像数据进行预测，得到输出 output。
        train_loss = nn.CrossEntropyLoss()(output,label).to(device)# 使用交叉熵损失函数计算预测输出和真实标签之间的损失。
        train_loss.backward()# 反向传播
        optim.step()# 优化器更新
        total_train += train_loss # 损失相加
    sculer.step() # 在每个 epoch 结束时，调用学习率调度器的 step() 方法来更新学习率。
    total_test = 0 # 总损失
    ...
    torch.save(net.state_dict(),"5catdogchikenhorsebutterfly{}.pth".format(epoch+1))
    print("模型已保存")
    print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())))
